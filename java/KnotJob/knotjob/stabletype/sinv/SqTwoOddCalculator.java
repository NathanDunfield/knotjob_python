 ...
 * @author Dirk
 */
public class SqTwoOddCalculator extends Calculator {
    
    private final int epsilon;
    
    public SqTwoOddCalculator(ArrayList<LinkData> lnkLst, Options optns, DialogWrap frm, int eps) {
    ...
    protected boolean calculationRequired(LinkData theLink) {
        ...
        if (epsilon == 0) return (theLink.sqtOdd == null);
        return theLink.sqtOde == null;
    }
    ...
    protected void performCalculation(LinkData theLink) {
        ...
        if (abInf.isAborted()) return;
        SqTwoOddInvariant sqInv = new SqTwoOddInvariant(theLink.chosenLink(), 
                theLink.sInvariant(2), cases, epsilon, frame, options);
        sqInv.calculate();
        if (!abInf.isAborted()) {
            if (epsilon == 0) theLink.sqtOdd = sqInv.getInvariant();
            else theLink.sqtOde = sqInv.getInvariant();
        }
    }
